...
//   Foul blocks incoming strikes and resets every turn.
...
var Game = {
  ...
  start: function (characterId) {
    ...
    this.state = {
      ...
      weightRoom: 0,      // pending Single -> Double upgrades
      stoleThisTurn: false,
  ...
  hit: function (basesAdvanced) {
    ...
    // Weight Room turns your next single into a double
    if (basesAdvanced === 1 && s.weightRoom > 0) {
      basesAdvanced = 2;
      s.weightRoom--;
      UI.log("All that time in the weight room.");
    }
  ...
  addFoul: function (amount) {
    if (amount > 0) {
      this.state.foul += amount;
    }
  },
  ...
  resolvePitch: function () {
    ...
    if (pitch.strikes > 0) {
      ...
      var blocked = pitch.ignoresFoul ? 0 : Math.min(s.foul, incoming);
      var through = incoming - blocked;

      if (pitch.ignoresFoul && s.foul > 0) {
        UI.log("Painted. Foul can't touch it.");
      }
      if (blocked > 0) {
        UI.log("Fouled off " + blocked + ".");
      }
      if (through > 0) {
